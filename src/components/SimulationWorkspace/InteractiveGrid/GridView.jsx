...
const GridView = ({ size, offset, zoom, showCoordinates = true }) => {
  const { w, h } = size;
  ...
  return (
    <svg width={size.w} height={size.h} className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      {/* Primary Horizontal Axis (The Ground) - Always Visible */}
      <line x1={0} y1={axisXY} x2={w} y2={axisXY} className="stroke-gray-400 dark:stroke-[#B5BAC1]" strokeWidth={4} />
      
      {/* Primary Vertical Axis - Only if coordinates are shown */}
      {showCoordinates && (
        <>
      ...
      {/* Labels */}
      {showCoordinates && labels.map((lbl) => (
        <text
          key={lbl.key}
          x={lbl.x}
          y={lbl.y}
