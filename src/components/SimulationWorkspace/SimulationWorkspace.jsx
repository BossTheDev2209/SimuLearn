              ...
              <InteractiveGrid initialCamera={activeSim.physicsState?.camera} onCameraChange={handleCameraChange} activeTool={activeTool} onGridClick={handleGridClick} onGridPointerDown={handleGridPointerDown} onGridPointerMove={handleGridPointerMove} onGridPointerUp={handleGridPointerUp}>
                {({ size, offset, zoom, unitStep }) => (
                  <>
                    ...
                    <AnimatePresence>
                      {vectorEditor && (
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="fixed z-[300] bg-white dark:bg-[#2B2D31] rounded-[10px] shadow-lg border border-theme-border p-2 flex flex-col gap-2" style={{ left: vectorEditor.screenX + 15, top: vectorEditor.screenY + 15 }}>
                          ...
                          <div className="flex flex-col gap-1.5 min-w-[140px]">
                            <div className="flex items-center gap-2 bg-[#F3F4F6] dark:bg-[#1E1F22] rounded-md px-2 py-1 relative pr-8">
                              ...
                              <input autoFocus type="number" className="w-full bg-transparent text-sm font-medium outline-none text-theme-primary" value={vectorEditor.magnitude} onChange={(e) => { const val = Number(e.target.value) || 0; setVectorEditor(prev => ({ ...prev, magnitude: val })); updateVectorValue(vectorEditor.objId, vectorEditor.type, vectorEditor.index, { magnitude: val }); }} />
                            ...
                            <div className="flex items-center gap-2 bg-[#F3F4F6] dark:bg-[#1E1F22] rounded-md px-2 py-1 relative pr-8">
                              ...
                              <input type="number" className="w-full bg-transparent text-sm font-medium outline-none text-theme-primary" value={vectorEditor.angle} onChange={(e) => { const val = Number(e.target.value) || 0; setVectorEditor(prev => ({ ...prev, angle: val })); updateVectorValue(vectorEditor.objId, vectorEditor.type, vectorEditor.index, { angle: val }); }} />
                          ...
                          <button onClick={() => setVectorEditor(null)} className="bg-theme-primary text-white text-xs font-bold py-1.5 rounded-md mt-1">ตกลง</button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </>
