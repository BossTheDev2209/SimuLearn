            ...
            <div className="flex-1 rounded-2xl overflow-hidden border border-theme-border bg-white dark:bg-[#2B2D31] relative shadow-sm">
              ...
              <div className="absolute bottom-4 right-4 z-40 flex justify-end pointer-events-none">
                <motion.div 
                  initial={false}
                  animate={{ 
                    width: isTimelineCollapsed ? 48 : 400,
                    height: isTimelineCollapsed ? 48 : 60,
                    borderRadius: isTimelineCollapsed ? 24 : 20
                  }}
                  transition={{ type: "spring", bounce: 0, duration: 0.4 }}
                  className="bg-white/90 dark:bg-[#313338]/90 backdrop-blur-md shadow-2xl border border-theme-border pointer-events-auto relative overflow-hidden max-w-[calc(100vw-32px)]"
                >
                  {/* ปุ่มตอนพับ (Collapsed State) */}
                  <button 
                    onClick={() => setIsTimelineCollapsed(false)}
                    className={`absolute right-0 top-0 w-12 h-12 flex items-center justify-center text-[#FFB65A] hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-300 z-10 ${
                      isTimelineCollapsed ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none scale-75'
                    }`}
                    title="ขยาย"
                  >
                    <ChevronLeftIcon />
                  ...
                  {/* เนื้อหาตอนขยาย (Expanded State) - ล็อคความกว้างไว้เพื่อไม่ให้โดนบีบตอนแอนิเมชัน */}
                  <div 
                    className={`absolute right-0 top-0 bottom-0 w-[400px] max-w-[calc(100vw-32px)] p-3 flex items-center gap-4 transition-all duration-300 ${
                      isTimelineCollapsed ? 'opacity-0 pointer-events-none translate-x-4' : 'opacity-100 pointer-events-auto translate-x-0'
                    }`}
                  >
                    <button 
