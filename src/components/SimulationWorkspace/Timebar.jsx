  ...
  return (
    <div className="relative font-['Chakra_Petch'] text-gray-800" onPointerDown={e => e.stopPropagation()}>
      <div className="flex flex-col gap-2 items-end">
        {/* Main Bar */}
        <div className="flex items-center bg-[#e8e2d5] dark:bg-[#313338] rounded-xl p-1 shadow-sm border border-[#d6cfbe] dark:border-[#1E1F22] h-10 transition-all">
          ...
          {isTimebarOpen && (
            <>
              <button 
                onClick={onRestart} 
                className="bg-[#FFB65A] hover:opacity-90 text-white p-1.5 rounded-lg mx-1.5 transition-all shadow-sm active:scale-95" 
                title="รีสตาร์ทเวลา"
              >
                <RestartIcon />
              </button>
              ...
              <button 
                ...
                className="hover:bg-[#dcd6c7] dark:hover:bg-[#3F4147] text-gray-800 dark:text-gray-300 p-1.5 rounded-lg mr-1.5 transition-colors active:scale-95" 
                title="เล่น/หยุด"
              >
                {isPlaying ? <PauseIcon /> : <PlayIcon />}
              </button>
              
              <div className="flex items-center bg-[#dcd6c7] dark:bg-[#3F4147] rounded-lg px-2.5 py-1 text-[11px] font-bold gap-3 shadow-inner h-full mr-1">
          ...
          <button 
            ...
            className="p-1 w-7 h-7 flex items-center justify-center hover:bg-[#dcd6c7] dark:hover:bg-[#3F4147] text-gray-800 dark:text-gray-300 rounded-lg transition-colors"
          >
        ...
        {isTimebarOpen && (
          <div className="flex items-center bg-[#e8e2d5] dark:bg-[#313338] text-gray-800 dark:text-gray-200 rounded-xl px-4 py-1.5 shadow-sm border border-[#d6cfbe] dark:border-[#1E1F22] h-9">
