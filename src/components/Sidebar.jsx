...
export default function Sidebar({ 
  ...
  // --- Logic เดิมของเคนโด้ทั้งหมด (คงไว้ 100%) ---
  useEffect(() => {
      ...
      <div className={`p-4 ${!isOpen && 'px-2'} flex flex-col items-center flex-1 overflow-hidden`}>
        ...
        <div className={`flex items-center w-full ${isOpen ? 'justify-between' : 'justify-center'} mb-8 mt-2`}>
          {isOpen && (
             <div className="flex items-center">
               <button onClick={onHomeClick} className="bg-[#FFAA44] text-white rounded-full w-9 h-9 flex items-center justify-center font-bold text-sm overflow-hidden mr-2 cursor-pointer hover:scale-110 active:scale-95 transition-transform">
        ...
        {isOpen && (
          <div className="w-full px-2 mt-2 flex-1 overflow-hidden flex flex-col">
            <button onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="w-full flex items-center justify-between text-[13px] font-bold text-[#D3A068] mb-3 px-2 py-1 group shrink-0">
              ...
              <ul className="space-y-[2px] text-[14px] text-gray-700 font-medium h-full overflow-y-auto custom-scrollbar" ref={menuRef}>
                {simulations.length === 0 ? (
                  ...
                  simulations.map((sim, index) => (
                    <li key={sim.id} onClick={() => { if (renamingId !== sim.id) onSelectSimulation(sim.id); }} className={`group/item relative cursor-pointer rounded-lg px-2 py-2 transition-colors duration-200 flex items-center justify-between ${activeSimId === sim.id ? 'bg-[#D5CBBD] text-black font-bold' : activeMenu === index ? 'bg-[#DCD5CB] text-black' : 'hover:bg-[#DCD5CB] hover:text-black'}`}>
                      {renamingId === sim.id ? (
                        <input ref={renameInputRef} type="text" value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleConfirmRename(); if (e.key === 'Escape') handleCancelRename(); }} onBlur={handleConfirmRename} onClick={(e) => e.stopPropagation()} className="flex-1 mr-1 bg-white/80 text-black text-[14px] font-medium rounded px-1.5 py-0.5 outline-none border border-[#D3A068]/50 focus:border-[#D3A068]" />
                      ) : ( <span className="truncate flex-1 mr-1">{sim.title}</span> )}
                      {renamingId !== sim.id && (
                        <button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === index ? null : index); }} className={`shrink-0 flex items-center justify-center w-7 h-7 rounded-full transition-all cursor-pointer ${activeMenu === index ? 'opacity-100 bg-black/10 text-gray-800' : 'opacity-0 group-hover/item:opacity-100 hover:bg-black/10 text-gray-500 hover:text-gray-800'}`}>
                      ...
                      {activeMenu === index && (
                        <div className="absolute right-0 top-full mt-1 w-[170px] bg-[#FAF6F0] border border-[#DCD5CB] rounded-xl shadow-xl z-50 py-1.5 text-[13px] font-medium overflow-hidden">
                          <button onClick={(e) => { e.stopPropagation(); handleShare(sim); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-[#EBE5DD] transition-colors cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v13"/><path d="m16 6-4-4-4 4"/><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/></svg><span>แชร์</span></button>
                          <button onClick={(e) => { e.stopPropagation(); handleStartRename(sim); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-[#EBE5DD] transition-colors cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/><path d="m15 5 4 4"/></svg><span>เปลี่ยนชื่อ</span></button>
